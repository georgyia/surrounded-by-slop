...
const q = async (...args: string[]) => {
  ...
  const code = await run(["query", "--root", root, ...args], ctx);
  return { code, out: ctx.out(), err: ctx.err() };
};

describe("query callers / callees", () => {
  it("lists the callers of a function", async () => {
    const { code, out } = await q("callers", "money");
    expect(code).toBe(0);
    expect(out).toContain("# callers of money");
    expect(out).toContain("place");
  });

  it("lists the callees of a function", async () => {
    const { code, out } = await q("callees", "main");
    expect(code).toBe(0);
    expect(out).toContain("place");
    expect(out).toContain("money"); // transitive
  });

  it("bounds callees with --depth", async () => {
    const shallow = await q("callees", "main", "--depth", "1");
...
describe("query importers", () => {
  it("lists the modules importing a file", async () => {
    const { code, out } = await q("importers", "src/money.ts");
    expect(code).toBe(0);
...
describe("query path", () => {
  it("prints the shortest chain between two symbols", async () => {
    const { code, out } = await q("path", "main", "money");
    expect(code).toBe(0);
    expect(out).toContain("main → place → money");
  });

  it("reports when there is no path", async () => {
    const { code, out } = await q("path", "money", "main");
    expect(code).toBe(0);
...
describe("query defs", () => {
  it("finds declarations by substring", async () => {
    const { code, out } = await q("defs", "place");
    expect(code).toBe(0);
...
describe("query slice", () => {
  it("returns the neighborhood of a symbol", async () => {
    const { code, out } = await q("slice", "place", "--depth", "1");
    expect(code).toBe(0);
...
describe("query --json", () => {
  it("emits canonical IR JSON for callers", async () => {
    const { code, out } = await q("callers", "money", "--json");
    expect(code).toBe(0);
  ...
  it("emits a JSON id array for path", async () => {
    const { out } = await q("path", "main", "money", "--json");
    const parsed = JSON.parse(out);
...
describe("query error handling", () => {
  it("exits 1 with suggestions on an unknown symbol", async () => {
    const { code, err } = await q("callers", "moneyy");
    expect(code).toBe(1);
  ...
  it("exits 2 on an unknown subcommand", async () => {
    const { code, err } = await q("neighbours", "money");
    expect(code).toBe(2);
    expect(err).toContain("unknown query");
  });

  it("exits 2 when a required operand is missing", async () => {
    const { code, err } = await q("callers");
    expect(code).toBe(2);
