...
/**
 * Golden-fixture harness for the Go adapter (`fixtures/go/*`), same
...
const fixturesRoot = fileURLToPath(new URL("../../fixtures/go", import.meta.url));
const update = process.env.UPDATE_FIXTURES === "1";
const require = createRequire(import.meta.url);

let adapter: LanguageAdapter;
beforeAll(async () => {
  adapter = await createGoAdapter({
...
function readInputs(directory: string): FileInput[] {
  ...
  if (single === undefined) {
    throw new Error(`go fixture ${directory} has neither project/ nor input.*`);
  }
...
describe.each(cases)("go fixture $name", ({ directory }) => {
...
it("the go fixture suite covers at least 15 shapes", () => {
