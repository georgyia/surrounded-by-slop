...
import { createRubyAdapter } from "./adapter.js";

/**
 * Golden-fixture harness for the Ruby adapter (`fixtures/ruby/*`), same
...
const fixturesRoot = fileURLToPath(new URL("../../fixtures/ruby", import.meta.url));
const update = process.env.UPDATE_FIXTURES === "1";
const require = createRequire(import.meta.url);

let adapter: LanguageAdapter;
beforeAll(async () => {
  adapter = await createRubyAdapter({
    runtime: fs.readFileSync(require.resolve("web-tree-sitter/web-tree-sitter.wasm")),
    ruby: fs.readFileSync(
      path.join(
...
function readInputs(directory: string): FileInput[] {
  ...
  if (single === undefined) {
    throw new Error(`ruby fixture ${directory} has neither project/ nor input.*`);
  }
...
describe.each(cases)("ruby fixture $name", ({ directory }) => {
...
it("the ruby fixture suite covers at least 15 shapes", () => {
...
it("the adapter declares honest capabilities", () => {
  ...
  expect(adapter.extensions).toEqual([".rb"]);
});
