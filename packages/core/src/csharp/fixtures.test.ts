...
import { createCSharpAdapter } from "./adapter.js";

/**
 * Golden-fixture harness for the C# adapter (`fixtures/csharp/*`), same
...
const fixturesRoot = fileURLToPath(new URL("../../fixtures/csharp", import.meta.url));
const update = process.env.UPDATE_FIXTURES === "1";
const require = createRequire(import.meta.url);

let adapter: LanguageAdapter;
beforeAll(async () => {
  adapter = await createCSharpAdapter({
...
function readInputs(directory: string): FileInput[] {
  ...
  if (single === undefined) {
    throw new Error(`csharp fixture ${directory} has neither project/ nor input.*`);
  }
...
describe.each(cases)("csharp fixture $name", ({ directory }) => {
...
it("the csharp fixture suite covers at least 15 shapes", () => {
