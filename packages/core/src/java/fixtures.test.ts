...
import { createJavaAdapter } from "./adapter.js";

/**
 * Golden-fixture harness for the Java adapter (`fixtures/java/*`), same
...
const fixturesRoot = fileURLToPath(new URL("../../fixtures/java", import.meta.url));
const update = process.env.UPDATE_FIXTURES === "1";
const require = createRequire(import.meta.url);

let adapter: LanguageAdapter;
beforeAll(async () => {
  adapter = await createJavaAdapter({
    runtime: fs.readFileSync(require.resolve("web-tree-sitter/web-tree-sitter.wasm")),
    java: fs.readFileSync(
      path.join(
...
function readInputs(directory: string): FileInput[] {
  ...
  if (single === undefined) {
    throw new Error(`java fixture ${directory} has neither project/ nor input.*`);
  }
...
describe.each(cases)("java fixture $name", ({ directory }) => {
...
it("the java fixture suite covers at least 15 shapes", () => {
