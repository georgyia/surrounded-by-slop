...
/**
 * Golden-fixture harness for the Rust adapter (`fixtures/rust/*`), same
...
const fixturesRoot = fileURLToPath(new URL("../../fixtures/rust", import.meta.url));
const update = process.env.UPDATE_FIXTURES === "1";
const require = createRequire(import.meta.url);

let adapter: LanguageAdapter;
beforeAll(async () => {
  adapter = await createRustAdapter({
    runtime: fs.readFileSync(require.resolve("web-tree-sitter/web-tree-sitter.wasm")),
    rust: fs.readFileSync(
      path.join(
...
function readInputs(directory: string): FileInput[] {
  ...
  if (single === undefined) {
    throw new Error(`rust fixture ${directory} has neither project/ nor input.*`);
  }
...
describe.each(cases)("rust fixture $name", ({ directory }) => {
...
it("the rust fixture suite covers at least 15 shapes", () => {
